...
- **Elite drums — remaining features** (MVP gem placement + colours + width + SP done; audit found no note-dropping bugs left):
  - **Roll/tremolo lanes** (MIDI 108–118) — not recognized in `TrackResolver::extract`, and `resolveLanes` has no elite branch (calls 4-lane `getDrumColumn`; dead code today, latent). Recognize elite roll pitches + add `getEliteDrumColumn` branch.
