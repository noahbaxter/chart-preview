...
- **Elite drums — remaining features.** Audited against the full spec (`_refs/midi/elite-drums.md`) on 2026-09-09. Gems, colours, width, SP, hi-hat Open/Closed/Indifferent, roll lanes and 9-lane authoring are done. What is left, roughly in order of how wrong it is today:
  ...
  - **Flam** (87/63/39/15) — modifier parsing plus a distinct gem. Retains ghost/accent and hat state, ignored inside roll lanes, does not apply to Kick/Stomp/Splash. Kick flams are separately notated as a simultaneous 1x + 2x kick and we currently draw those as two independent bars. Temp art for both: two gems squished horizontally to fit one lane.
