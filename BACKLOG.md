...
- **Elite drums — remaining features** (MVP gem placement + colours + width + SP done; audit found no note-dropping bugs left):
  ...
  - **Purple L-Crash gem art**: currently borrows the blue cymbal placeholder; needs real purple art (or the procedural glyph below).
...
- **Procedural note/cymbal gem glyphs** — Replace the PNG gem assets with procedural rendering (same approach as the procedural strikeline pads). Enables customizable per-lane colors, removes the purple L-Crash tap-note placeholder, and gives more shape options. Input = `ELITE_LANE_STYLES` tints (make user-editable for custom colors). Generalizes to guitar/4-lane drum gems, not just elite.
