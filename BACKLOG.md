...
- **Elite drums — remaining features.** Audited against the full spec (`_refs/midi/elite-drums.md`) on 2026-09-09. Gems, colours, width, SP, hi-hat Open/Closed/Indifferent, roll lanes and 9-lane authoring are done. What is left, roughly in order of how wrong it is today:
  ...
  - **Flam overlays sit wrong (open)** — ghost rings and accent chevrons are mispositioned and undersized on a flam, worst on the ghost gems where the ring barely shows. `applyCurvedImageSwap` derives the overlay's height from `args.overlayBaseW`, which for a two-overlay flam is the SQUISHED sub-lane width, so the overlay shrinks with the squish while the gem keeps full-lane height (the gem passes `laneStrikeWidth`). Likely fix: pass `laneStrikeWidth` as `overlayBaseW` unconditionally and leave `overlayAnchorX` on the sub-lane centre, so the overlay squishes by sprite width only, same as the gem.
