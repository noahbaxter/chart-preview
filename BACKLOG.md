...
- **Elite drums — remaining features** (MVP gem placement + colours + width + SP done; audit found no note-dropping bugs left):
  - **Roll/tremolo lanes** (MIDI 108–118) — not recognized in `TrackResolver::extract`, and `resolveLanes` has no elite branch (calls 4-lane `getDrumColumn`; dead code today, latent). Recognize elite roll pitches + add `getEliteDrumColumn` branch. Watch the SP collision: `extract`'s star-power test (`pitch == Guitar::SP`, i.e. 116) matches BEFORE the elite roll-lane check, so `ROLL_TOM3` (116) would be collected as a phantom SP phrase. Order the elite roll check ahead of the SP test, or guard the SP test with `!isElite` for 116.
  - **Stomp/Splash pedal GEMS** → the Pedal Down note (72) now parses as the Closed-hat modifier, but its dual-purpose Stomp/Splash *gem* generation (velocity 2-126 = Stomp, 127 = Splash) is not wired: cols 10/11 (reserved in `LANE_COUNT=12`) still have no MIDI source. Needs the velocity→gem rule + `108` Stomp/Splash roll lane + `[STRICT_HAT_PEDAL_STATE]`. Art + rendering already exist (`bar_stomp`, cols 10/11 render).
  - **Hi-hat sustains** (Open/Splash generate ringing sustains, terminated by Pedal Down / Closed / Open) — not parsed. Separate from the Open/Closed/Indifferent gem state, which IS done.
  - **Flam** (upper-octave Eb: 87/63/39/15) — modifier parsing + a distinct flam gem/animation. No flam art exists. (Hi-hat Open/Closed/Indifferent parsing + rendering is DONE.)
  ...
  - **Elite authoring/write-mode**: `HitTestMapper` uses 4-lane drum geometry for elite (mouse edits misaligned); `is2xKickLane`/`getKickSide`/`columnToDrumPitch` hardcode 4-lane col 6 (should honour `ELITE_KICK_2X_COLUMN=9`). Latent until elite editing lands.
